...
export const getApiUrl = (): string => {
  ...
  if (typeof window !== 'undefined') {
    ...
    // Alibaba Cloud Production (hotelsogo-ai.com)
    if (hostname === 'tms-chat.hotelsogo-ai.com') {
      apiUrlCache = 'https://tms-chat.hotelsogo-ai.com/api/v1';
      log.debug('[Runtime Config] ✅ Detected Alibaba Cloud Production, setting cache to:', apiUrlCache);
      return apiUrlCache;
    }

    // Alibaba Cloud Staging
    if (hostname === 'tms-chat-staging.hotelsogo-ai.com') {
      apiUrlCache = 'https://tms-chat-staging.hotelsogo-ai.com/api/v1';
  ...
  return 'https://tms-chat-staging.hotelsogo-ai.com/api/v1';
};

/**
 * Get WebSocket base URL based on runtime hostname detection
 *
 * @returns WebSocket base URL (e.g., "wss://tms-chat-staging.hotelsogo-ai.com")
 */
export const getWebSocketUrl = (): string => {
  if (typeof window !== 'undefined') {
    ...
    if (hostname === 'tms-chat.hotelsogo-ai.com') {
      return 'wss://tms-chat.hotelsogo-ai.com';
    }

    // Alibaba Cloud Staging
    if (hostname === 'tms-chat-staging.hotelsogo-ai.com') {
      return 'wss://tms-chat-staging.hotelsogo-ai.com';
    }
