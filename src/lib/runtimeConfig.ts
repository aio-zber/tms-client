...
export const getApiUrl = (): string => {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (hostname === 'localhost' || hostname === '127.0.0.1') {
      ...
      console.log('[Runtime Config] Detected localhost, using:', apiUrlCache);
      return apiUrlCache;
    }
    ...
    if (hostname.includes('railway.app')) {
      ...
      console.log('[Runtime Config] Detected Railway deployment, using:', apiUrlCache);
      return apiUrlCache;
    }

    // Production or unknown domain - use staging as fallback
    apiUrlCache = 'https://tms-server-staging.up.railway.app/api/v1';
    console.log('[Runtime Config] Unknown hostname, using default:', apiUrlCache);
    return apiUrlCache;
  }
